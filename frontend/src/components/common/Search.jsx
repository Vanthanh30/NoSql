...
const Search = () => {
    ...
    const [searchResults, setSearchResults] = useState({ courses: [], articles: [] });
    const [showSearchResults, setShowSearchResults] = useState(false);
    const [isSearching, setIsSearching] = useState(false);

    // Xử lý click bên ngoài search results
    useEffect(() => {
    ...
    // Hàm tìm kiếm local trong danh sách courses và articles
    const searchAPI = async (query) => {
        try {
            setIsSearching(true);

            // Gọi cả 2 service song song
            ...
            return {
                courses: filteredCourses.slice(0, 5), // Giới hạn 5 kết quả
                        ...
                        <div className="search-section">
                            ...
                            {searchResults.courses.map((course) => (
                                <div
                                    ...
                                    onClick={() => handleSearchItemClick('course', course._id || course.id)}
                                >
                                    {course.thumbnail && (
                                        <img
                        ...
                        <div className="search-section">
                            ...
                            {searchResults.articles.map((article) => (
                                <div
                                    ...
                                    className="search-result-item"
                                    onClick={() => handleSearchItemClick('article', article._id || article.id)}
                                >
                                    {article.thumbnail && (
                                        <img
