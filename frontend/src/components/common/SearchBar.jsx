...
const SERVICE_MAP = {
    ...
    course: async (keyword) => {
        ...
        const courses = Array.isArray(res) ? res : res.data?.courses || res.data || [];
        return courses
            .filter(course =>
    ...
    article: async (keyword) => {
        ...
        const articles = Array.isArray(res) ? res : res.data?.articles || res.data || [];
        return articles
            .filter(article =>
    ...
    category: async (keyword) => {
        ...
        const categories = Array.isArray(res) ? res : res.data?.categories || res.data || [];
        return categories
            .filter(cat =>
            ...
            {results.length > 0 && (
                ...
                >
                    {results.map(item => (
                        ...
                        >
                            <span>
                                {item._type === "account" ? item.fullName :
                                    item._type === "course" ? item.title :
                                        item._type === "article" ? item.title :
                                            item.name}
                            </span>
