...
function EditCourse() {
  ...
  const loadCourse = async () => {
    try {
      ...
      setStartDate(
        data.time?.startDate ? data.time.startDate.split("T")[0] : ""
      );
      ...
      if (originalChapters.length > 0) {
        const chaptersWithLessons = await Promise.all(
          originalChapters.map(async (ch) => {
            ...
            try {
              const { data: chapterDetail } = await chapterService.getById(
                chapterId
              );
              ...
              const lessonsDetailed = await Promise.all(
                (chapterDetail.lessons || []).map(async (lesson) => {
                  ...
                  try {
                    const { data: lessonDetail } = await lessonService.getById(
                      lessonId
                    );
                    return {
                      ...lessonDetail,
      ...
      if (formRef.current) {
        ...
        if (img)
          img.src = data.media?.imageUrl || "https://via.placeholder.com/150";
  ...
  const handleLessonVideoChange = (chIdx, lesIdx, file) => {
    ...
    if (!lessonId) {
      alert(
        'Bài học này chưa có ID trong CSDL. Hãy bấm "Lưu khóa học" trước khi tải video.'
      );
      return;
    }
  ...
  const handleStartDateChange = (e) => {
    ...
    if (endDate && val > endDate) {
      alert("Ngày bắt đầu không được trễ hơn ngày kết thúc hiện tại!");
      return;
    }
